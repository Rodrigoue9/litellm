    ...
    <Table>
      <TableHeader>
        <TableRow>
          ...
          {["Judged turns", "Router wins", `${otherArmLabel(job.direction)} wins`].map((label) => (
            <TableHead key={label} className="text-right">
              {label}
            </TableHead>
          ))}
...
const ResultsBody: React.FC<{ job: ShadowEvalJob; resultsError?: boolean }> = ({ job, resultsError = false }) => {
  ...
  return (
    <>
      ...
      {job.keys.length > 1 && <KeyTable job={job} />}
      {results.by_current_model.length > 0 && (
        <div className={job.keys.length > 1 ? "border-t" : ""}>
          <SliceTable
      ...
      {results.by_tier.length > 0 && (
        <div className={results.by_current_model.length > 0 || job.keys.length > 1 ? "border-t" : ""}>
