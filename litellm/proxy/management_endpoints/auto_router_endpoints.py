...
async def list_shadow_eval_jobs(
    ...
    if prisma_client is None:
        raise HTTPException(status_code=500, detail=CommonProxyErrors.db_not_connected_error.value)
    heads: Final = await prisma_client.db.litellm_shadowevaljob.find_many(
        where={"api_key_id": api_key_id} if api_key_id else {},  # mutable-ok: Prisma filter
        order={"created_at": "desc"},  # mutable-ok: Prisma order
        distinct=["group_id"],  # mutable-ok: Prisma distinct
        take=limit,
    )
    group_ids: Final = tuple(str(head.group_id) for head in heads or ())
    ...
    by_group: Final[Mapping[str, tuple[_LegRow, ...]]] = MappingProxyType(
        {group_id: tuple(leg for leg in legs if leg.group_id == group_id) for group_id in group_ids}
    )
