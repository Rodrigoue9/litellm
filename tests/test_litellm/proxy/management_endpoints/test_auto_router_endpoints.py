...
def _shadow_prisma(legs=(), agg_rows=None, by_leg_rows=None, known_keys=("key-hash", "key-hash-2")) -> MagicMock:
    ...
    async def find_many_legs(where=None, order=None, distinct=None, take=None, **_: object):
        current = list(stored)
        w = dict(where or {})
        if "api_key_id" in w:
        ...
        if order:
            current = sorted(current, key=lambda row: row.created_at, reverse=True)
        if distinct == ["group_id"]:
            deduped: dict = {}
            for row in current:
                deduped.setdefault(row.group_id, row)
...
async def test_list_shadow_eval_jobs_collapses_legs_into_jobs_newest_first(monkeypatch: pytest.MonkeyPatch):
    ...
    head_call = prisma.db.litellm_shadowevaljob.find_many.await_args_list[0].kwargs
    assert head_call["distinct"] == ["group_id"]
    assert head_call["take"] == 50
